...
The following are example Schemas and [Objects](/docs/highway/modules/objects.mdx) which outline how types declared within a schema map to the objects which are defined from them. Below are two examples: An `User Status`, and a `Pet`.
