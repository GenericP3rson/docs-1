...
1. Add the `get_storage` package to our project:

  ```bash title="terminal"
  flutter pub add get_storage
  ```

1. Import the `get` and `get_storage` package in our `main.dart` file:
...
1. Change the `MyApp` class to return `GetMaterialApp` instead of `MaterialApp`:
...
To do this, we will utilize the `ContinueOnSonr Button` that exists within the `motor_flutter` package.
...
The login function returns a `Future<WhoIs>`.
A `WhoIs` is a data structure that contains information about the user. 
...
You can just copy `main.dart` and `login_page.dart` from the [todo app](https://github.com/ntindle/todo_app) and build from there too.
:::
...
Future<void> _createSchema() async {
  if (!Get.isRegistered<Schema>(tag: "taskSchema")) {
      reateSchemaResponse resp = await MotorFlutter.to.publishSchema("task", {
...
This is where we actually register the `Schema` object in memory.
`Schema` are used to create new instances of the schema using `newDocument`.
...
```
This function pulls the `Schema` `taskSchema` from memory, creates a new document, sets the `task` field to the text in the text field, and then uploads it.
