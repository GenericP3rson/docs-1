...
```
This function pulls the `SchemaDefinition` "taskSchema" from memory, creates a new document, sets the `task` field to the text in the text field, and then saves it.
If the save is successful, we get a `SchemaDocument` back, which we can use to get the DID of the new document.
We then clear the text field and show a success message.
If the save fails, we show an error message and do not clear the text field.